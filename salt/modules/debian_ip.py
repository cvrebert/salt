...
def _write_file_ifaces(iface, data, **settings):
    ...
    for adapter in adapters:
        ...
        if 'type' in adapters[adapter] and adapters[adapter]['type'] == 'source':
            tmp = source_template.render({'name': adapter, 'data': adapters[adapter]})  # pylint: disable=E1103
        else:
            tmp = eth_template.render({'name': adapter, 'data': adapters[adapter]})  # pylint: disable=E1103
        ifcfg = tmp + ifcfg
        if adapter == iface:
            saved_ifcfg = tmp
